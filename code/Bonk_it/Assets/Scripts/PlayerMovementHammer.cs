...
public class PlayerMovementHammer : MonoBehaviour
{
    ...
    */
    ...
    [SerializeField] private GameObject movementGrapple;
    ...
    private void Update()
    {
        if (Input.GetButtonDown("JumpHammer") && isGrounded && Time.timeScale > 0) jumpNextFrame = true;

        MovementNextFrame = new Vector3(Input.GetAxis("MovementHorizontalHammer"), 0f, Input.GetAxis("MovementVerticalHammer"));
        ...
        */
        ...
        if ((Input.GetAxis("MovementHorizontalHammer") > 0) || Input.GetAxis("MovementVerticalHammer") > 0)
        {
    ...
    private void WalkingSound()
    {
        ...
        {
            playingSound = false;
            if (!movementGrapple.GetComponent<PlayerMovementGrapple>().walkingSound)
            {
                walking.Stop();
            }
        }
